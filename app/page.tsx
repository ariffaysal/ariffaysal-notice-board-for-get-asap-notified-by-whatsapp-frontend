...
export default function Home() {
  ...
  const handleDeleteAll = async () => {
    if (confirm("⚠️ ARE YOU SURE? This will permanently delete ALL notices.")) {
      if (confirm("Final warning: Clear everything?")) {
        try {
    ...
    <main className="max-w-4xl mx-auto p-6 bg-gray-50 min-h-screen relative pb-20 text-black">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 bg-white shadow-sm p-6 rounded-2xl border mb-12">
        ...
        <textarea 
          ...
          required
        />
        <button disabled={loading} className="w-full py-3 rounded-xl font-bold text-white bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 transition">
            ...
            <div key={notice.id} className="p-5 bg-white border rounded-2xl shadow-sm border-l-4 border-l-blue-500 hover:shadow-md transition">
              <div className="flex justify-between items-start mb-3">
                ...
                <div className="flex gap-2">
                  ...
                  <button onClick={() => setReplyTarget(notice)} className="bg-green-600 text-white px-4 py-2 rounded-xl text-sm font-medium hover:bg-green-700 shadow-sm">Reply</button>
      ...
      {replyTarget && (
        <div className="fixed bottom-10 right-10 w-80 bg-white shadow-2xl rounded-2xl border border-gray-200 overflow-hidden z-50 animate-in slide-in-from-bottom duration-300">
          <div className="bg-green-600 p-4 flex justify-between items-center text-white">
            {/* UPDATED: replyTarget.name */}
            <span className="text-xs font-bold uppercase truncate">Reply to: {replyTarget.name}</span>
            <button onClick={() => setReplyTarget(null)} className="hover:bg-green-700 p-1 rounded">✕</button>
          </div>
          <div className="p-4">
            <textarea value={commonReply} onChange={(e) => setCommonReply(e.target.value)} placeholder="Type your WhatsApp reply..." className="w-full p-3 border rounded-xl text-black text-sm outline-none focus:ring-2 focus:ring-green-500 h-32 resize-none bg-gray-50 shadow-inner" autoFocus />
            <button onClick={handleSendReply} className="w-full mt-3 bg-green-600 text-white py-3 rounded-xl font-bold hover:bg-green-700 transition shadow-lg">Send to WhatsApp</button>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
          <div className="bg-white p-6 rounded-3xl max-w-md w-full shadow-2xl">
            <div className="flex justify-between items-center mb-4 text-black">
              {/* UPDATED: selectedNotice.name */}
              <h2 className="text-xl font-bold">{selectedNotice.name}</h2>
            ...
            <div className="bg-gray-50 p-5 rounded-2xl border mb-6 max-h-60 overflow-y-auto">
              {/* UPDATED: selectedNotice.message */}
              <p className="text-gray-700 text-sm leading-relaxed whitespace-pre-wrap">{selectedNotice.message}</p>
            </div>
            <div className="flex gap-3">
               <button onClick={() => { setSelectedNotice(null); setReplyTarget(selectedNotice); }} className="flex-1 bg-green-600 text-white py-3 rounded-xl font-bold hover:bg-green-700 transition shadow-md">Reply</button>
